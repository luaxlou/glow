## 仓库职责说明

本手册仅覆盖 **glow 框架仓**（starter/SDK）。运维控制面（glow-server/glow-cli）已拆分到 **glow-ops** 仓库。

# Glow SDK 用户手册

`glow/starter` 是 Glow 框架的 Go SDK，提供应用内能力接入组件。

## 1. 安装

```bash
go get github.com/luaxlou/glow/starter
```

## 2. 快速开始

```go
package main

import (
    "fmt"
    "github.com/luaxlou/glow/starter/glowconfig"
)

func main() {
    logLevel := glowconfig.GetString("log_level")
    maxConnections := glowconfig.GetInt("max_connections")

    fmt.Printf("Log Level: %s\n", logLevel)
    fmt.Printf("Max Connections: %d\n", maxConnections)
}
```

## 3. 核心组件

### 3.1 glowconfig

从本地配置文件读取配置。

- `Get(key string) any`
- `GetString(key string) string`
- `GetInt(key string) int`
- `GetBool(key string) bool`
- `Reload() error`
- `SetConfigPath(path string)`

默认读取 `config.json`。

示例：

```json
{
  "log_level": "debug",
  "max_connections": 100,
  "mysql_dsn": "root:password@tcp(localhost:3306)/app",
  "redis_addr": "localhost:6379"
}
```

### 3.2 glowhttp

提供 HTTP 服务器启动适配（Gin）。

- `Init(port int)`
- `Router() *gin.Engine`
- `Run()`

### 3.3 glowmysql

提供 MySQL 连接初始化能力。

- `Init(name string)`
- `Gorm() (*gorm.DB, error)`
- `DB() (*sql.DB, error)`
- `Reload()`

### 3.4 glowredis

提供 Redis 客户端初始化能力。

- `Init()`
- `Client() (*redis.Client, error)`
- `Reload()`

### 3.5 glowsqlite

提供 SQLite 本地存储能力。

- `Init(path string)`
- `RegisterSchema(sql string)`
- `DB() (*sql.DB, error)`
- `Reload()`

## 4. 运行模式

### 4.1 本地开发

直接在应用目录准备 `config.json` 并运行应用。

### 4.2 嵌入式运行

Starter 仅负责应用内能力接入，不负责运维生命周期编排。

## 5. Starter 与运维生命周期边界（重要）

`starter/*` 仅负责应用内能力接入，不负责运维生命周期编排。

- Starter 负责：配置读取、HTTP 启动适配、数据库/缓存连接初始化
- 运维生命周期负责：`start/stop/restart/health/rollback` 编排
- 生命周期编排统一在 `glow-ops` 仓库实现
